...
const noteSlice = createSlice({
  ...
  reducers: {
    ...
    createOrUpdateNote: (state, action: PayloadAction<{ folderId: string, note: Note }>) => {
      const index = state.findIndex(folder => folder.id === action.payload.folderId)
      const potentialNote = action.payload.note

      const noteIndex = state[index].notes.findIndex(note => note.id === potentialNote.id)
      if (noteIndex >= 0) {
...
export const noteMiddleware = (store: MiddlewareAPI) => (next: Dispatch) => (action: Action) => {
